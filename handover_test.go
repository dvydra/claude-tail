...
func TestFlattenTodayKeepsTodayLocalOnly(t *testing.T) {
	...
	tree := sessionTree{Home: "/h", Folders: []treeFolder{{
		...
		Sessions: []treeSession{
			...
			{ID: "c", Path: "", Mtime: today, cwd: "/h/src/repo"}, // no transcript locator
		},
	}}}
...
func TestManifestSessionFromAmp(t *testing.T) {
	it := handoverItem{Agent: AgentAmp, SessionID: "T-amp", Repo: "o/r", Cwd: "/c", Title: "work", LastActivity: 1}
	ms := manifestSessionFrom(it, nil)
	if ms.Agent != "amp" || ms.TranscriptPath != "https://ampcode.com/threads/T-amp" {
		t.Fatalf("manifest session = %+v", ms)
	}
}
